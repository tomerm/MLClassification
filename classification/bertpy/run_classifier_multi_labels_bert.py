...
FLAGS = flags.FLAGS
#target = open("c:\BERT\sen1.txt", "w", encoding="utf-8")
...
flags.DEFINE_integer("num_classes", 40, "Total number of labels for sentiment analysis") #sen update second param: 80 -> 40
flags.DEFINE_integer("num_aspects", 40, "Total number of aspect")  #sen update second param: 20 -> 40
...
class DataProcessor(object):
  ...
  def _read_tsv(cls, input_file, quotechar=None):
    ...
    with tf.gfile.Open(input_file, "r") as f:
      ...
      for line in reader:
        lines.append(line)
      print ("Got %d lines from %s"%(len(lines), input_file))
      print ("Call _read_tsv(), which returns %d lines."%(len(lines)))           
      return lines
...
class SentimentAnalysisFineGrainProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    print ("Call get_train_examples() from SentimentAnalysisFineGrainProcessor. data_dir is %s"%(data_dir))
  ...
  def get_labels(self):
    """See base class."""
    label_list=[]
    path = "/home/user/MLClassificationData/train/rtanews/source"
    os.chdir(path)
    for f in glob.glob("*"):
        if os.path.isdir(f):
            label_list.append(f.replace(".",""))
  ...
  def _create_examples(self, lines, set_type):
    ...
    print ("_create_examples() returns %d examples."%(len(examples)))
    return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  #strsen1 = "\n!!SEN!! convert_single_example %d, text_a: [%s], text_b: [%s]"%(ex_index,example.text_a,example.text_b)
  #if ex_index < 3:
  #  target.write(strsen1 + "\n")
  ...
  if ex_index < 5:    #sen Update 5 -> 0
    ...
    target.write("\nlabel: %s (id = %s)" % (str(example.label), str(label_id))) # %d
  '''
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    if ex_index % 1000 == 0 or ex_index == len(examples)-1:
...
def main(_):
  ...
  print ("Processor: %s"%(task_name))
  processor = processors[task_name]()

  print ("Get label_list")
